...
<http://www.cs.ucsb.edu/~zmatni/cs16/lab08/code>

You should see a listing of several C++ programs. We are going to copy those into your ~/cs16/lab08 directory all at once with the following command:

`cp ~zmatni/public_html/cs16/lab08/code/* ~/cs16/lab08`

Note: If you get the error message:
`cp: target /cs/student/youruserid/cs16/lab05 is not a directory`

then it probably means you did not create a ~/cs16/lab08 directory in step 1, so go back to step 1 and do that first.
The * symbol in this command is a "wildcard" — it means that we want <b>all</b> of the files from the source directory copy be copied into the destination directory namely ~/cs16/lab08.

After doing this command, if you cd into ~/cs16/lab08 and use the ls command, you should see several files in your ~/cs16/lab08 directory—the same ones that you see if you visit the link http://www.cs.ucsb.edu/~zmatni/cs16/lab08/code
...
```
-bash-4.2$ make clean
/bin/rm -f arrayToStringTest arrayBoundsDemo countEvensTest minOfArrayTest minOfArrayErrorTest countPrimesTest maxOfArrayTest maxOfArrayErrorTest sumOddsTest sumOfArrayTest utilityTest *.o
...
|maxOfArrayErrorTest|<b>*YES*</b>|Run this, and you will see that initially there is no output. That is because the main is empty. YOU NEED TO REPLACE THIS MAIN with code that tests maxOfArray when a size of zero is passed in Use maxOfArrayErrorTest as a model.
